...
export function MilitaryScreen() {
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      >
        <ul className="flex flex-col gap-1">
          {ranking.map((entry, index) => (
            <li key={entry.id} className="flex items-center gap-2.5 text-[12px]">
              <span className="tnum w-4 text-[var(--color-ink-faint)]">{index + 1}</span>
              <Flag country={entry.id} size={20} />
              <span className="flex-1">{entry.name}</span>
              <span className="h-1.5 w-24 overflow-hidden rounded-full bg-[var(--color-surface-muted)]">
                <span
              ...
              <span className="tnum w-10 text-right font-medium">{entry.power.toFixed(0)}</span>
            </li>
          ))}
        </ul>
      </Panel>
      ...
      >
        {game.wars.length === 0 ? (
          <p className="text-xs text-[var(--color-ink-faint)]">진행 중인 전쟁이 없습니다.</p>
        ) : (
            ...
            {game.wars.map((war, index) => {
              ...
              return (
                <li key={`${war.attacker}-${war.defender}-${index}`}>
                  ...
                  <div className="mt-2">
                    <span className="mb-1 block text-[10px] text-[var(--color-ink-faint)]">
                      전선 (오른쪽으로 갈수록 공격측 우세)
                    </span>
