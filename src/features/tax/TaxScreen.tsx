...
export function TaxScreen() {
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
      <Panel title="세율" description={`한 분기에 세목당 ±${limit}%p 까지 조정할 수 있습니다.`}>
      ...
      <div className="flex flex-col gap-5">
          ...
          <table className="mt-4 w-full text-[12px]">
            <thead>
              <tr className="text-[10px] text-[var(--color-ink-faint)]">
                <th className="pb-1.5 text-left font-medium">세목</th>
                <th className="pb-1.5 text-right font-medium">현재 세수</th>
                <th className="pb-1.5 text-right font-medium">변경 후</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              <RevenueRow label="소득세" current={current.income} next={projected.income} />
              <RevenueRow label="법인세" current={current.corporate} next={projected.corporate} />
              <RevenueRow
                label="소비세"
                current={current.consumption}
                next={projected.consumption}
              />
              <RevenueRow label="세외수입" current={current.other} next={projected.other} />
              <RevenueRow label="합계" current={current.total} next={projected.total} strong />
            </tbody>
          </table>
        ...
        <Panel title="재정수지 추이" description="연율 기준입니다. 0 아래는 적자입니다.">
          <LineChart
            labels={labels}
            includeZero
            height={200}
...
function RevenueRow({
  ...
  return (
    <tr className={strong ? 'font-semibold' : ''}>
      ...
      <td className="tnum py-1.5 text-right">{formatMoney(current)}</td>
